...
generally, to log from a module, you should do the following:
    ...
    from adroll.utils.log_patch import logger
...
def log(level, message, **kwargs):
    ...
    return getattr(logger, 'original_'+level)(prefix+message, **kwargs)

def set_default_logger(name):
    """
    allow other processes (eg, spaceman, logman)
    ...
    def log_level(level):
        return lambda message, **kwargs: log(level, message, **kwargs)
    ...
    for level in ['info','warn','warning','error']:
        lvl = '%s' % level
        setattr(logger, 'original_'+level, getattr(logger, level))
        setattr(logger, level, log_level(level))

    return logger

set_default_logger("vanillons")
